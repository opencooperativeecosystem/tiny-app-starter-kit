...
var s=document.querySelector("script[data-namespace='resources-flow']");if(s){publicPath=s.getAttribute('data-path');}
...
(function(window, document, Context, appNamespace, publicPath) {
  ...
  function initStyleTemplate(domApi, cmpConstructor) {
    ...
    if (style) {
      ...
      var styleModeId = cmpConstructor.is + (cmpConstructor.styleMode || '');
      if (!cmpConstructor[styleModeId]) {
        ...
        cmpConstructor[styleModeId] = style;
      }
    }
  }
  function attachStyles(domApi, cmpConstructor, modeName, elm, customStyle, styleElm) {
    ...
    var styleModeId = cmpConstructor.is + (modeName || '');
    var styleTemplate = cmpConstructor[styleModeId];
    if (!styleTemplate) {
    ...
    if (styleTemplate) {
      ...
      if (!appliedStyles[styleModeId]) {
        ...
        (true, customStyle) && customStyle.addStyle(styleElm);
  ...
  function render(plt, elm, cmpConstructor, isUpdateRender) {
    try {
      ...
      // note that we do not create the host element cuz it already exists
            var hostMeta_1 = cmpConstructor.host;
      ...
      plt.attachStyles(plt.domApi, cmpConstructor, instance_1.mode, elm);
 ...
 // Vnode might have been moved left
  ...
  function createPlatformClientLegacy(Context, App, win, doc, publicPath, hydratedCssClass) {
    ...
    function loadBundle(cmpMeta, modeName, cb, bundleId) {
      bundleId = cmpMeta.bundleIds[modeName] || cmpMeta.bundleIds;
    ...
    true;
    plt.attachStyles = function(domApi, cmpConstructor, modeName, elm) {
...
})({},"ResourcesFlow","hydrated","/build/resources-flow/");